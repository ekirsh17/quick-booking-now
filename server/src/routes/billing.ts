 ...
 */
router.post('/paypal/create-subscription', async (req: Request, res: Response) => {
  try {
    const body = z.object({
      merchantId: z.string().uuid(),
      planId: z.enum(['starter', 'pro']),
 ...
 * Captures a PayPal subscription after approval
 */
router.get('/paypal/capture', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to capture PayPal subscription' });
  }
});
