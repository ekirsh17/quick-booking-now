...
export function useOnboarding(): UseOnboardingReturn {
    ...
    async function checkOnboardingStatus() {
      ...
      try {
        const { data: profile, error } = await supabase
          .from('profiles')
          .select('onboarding_completed_at, onboarding_step, time_zone')
          .eq('id', user.id)
          .single();
  ...
  const nextStep = useCallback(async () => {
    ...
    if (currentStep === 2) {
      // Save timezone when leaving step 2
      await saveTimezone(timezone);
    }
    
    if (currentStep === 3) {
      // Seed default presets when leaving step 3
      await seedDefaultPresets();
    }
    
    setCurrentStep(nextStepNum);
    await saveStepProgress(nextStepNum);
  }, [currentStep, timezone, saveTimezone, seedDefaultPresets, saveStepProgress]);
  ...
  }, [user, timezone, saveTimezone, seedDefaultPresets, navigate, toast]);
