...
export function parseSchedulicityEmail(input: SchedulicityInput): ParsedCancellation[] | null {
  ...
  const isSchedulicity = /@[^>\s]*schedulicity\.com/i.test(fromAddress) || normalized.includes("schedulicity");
  if ((!hasCancelSignal && !hasRescheduleSignal) || !isSchedulicity) return null;
